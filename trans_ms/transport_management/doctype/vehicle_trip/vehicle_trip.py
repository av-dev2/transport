...
class VehicleTrip(Document):
	def onload(self):
		...
		if self.transporter_type not in ["Sub-Contractor", "Self Drive"] and self.get("main_route"):
			consumption = frappe.db.get_value("Vehicle", self.get("vehicle"), "fuel_consumption")
			route = frappe.db.get_value("Trip Route", self.get("main_route"), "total_distance")
			approved_fuel = consumption * route
			self.set("main_approved_fuel", str(approved_fuel) + " Litres")
		
		#Load approved fuel for return trip
		if self.transporter_type not in ["Sub-Contractor", "Self Drive"] and self.get("return_route"):
			consumption = frappe.db.get_value("Vehicle", self.get("vehicle"), "fuel_consumption")
			route = frappe.db.get_value("Trip Route", self.get("return_route"), "total_distance")
			approved_fuel = consumption * route
	...
	def validate_fuel_requests(self):
		...
		if make_request:
			existing_fuel_request = frappe.db.get_value("Fuel Request", 
...
def create_vehicle_trip(**args):
	...
	existing_vehicle_trip = frappe.db.get_value("Vehicle Trip", 
		{"reference_doctype": args.reference_doctype, "reference_docname": args.reference_docname})
		
	existing_return_trip = frappe.db.get_value("Vehicle Trip", 
		{"return_reference_doctype": args.reference_doctype, "return_reference_docname": args.reference_docname})
...
def make_vehicle_inspection(source_name, target_doc=None, ignore_permissions=False):
	...
	docs = get_mapped_doc("Vehicle Trip", source_name, {
		"Vehicle Trip": {
	...
	}, target_doc, postprocess=None, ignore_permissions=ignore_permissions)
...
def check_trip_status(**args):
	...
	if existing_trip:
		...
		if status != 'Closed':
			frappe.msgprint("Cannot Close the File because it's Trip is not closed,Please Create the Trip")
		else:
			return status
	else:
